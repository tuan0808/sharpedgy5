...
export class PredictionsService extends BaseApiService<Prediction> {
  ...
  private async fetchInitialPredictions(userId: string): Promise<void> {
    try {
      const initialPredictions = await firstValueFrom(
          (await this.getPredictions(userId)).pipe(
              catchError(error => {
  ...
  private async executeRequest<R>(request: Observable<R>, errorMsg: string): Promise<Observable<R>> {
    this.isLoading.set(true);
    this.errorMessage.set(null);
    ...
    try {
      const token = await this.auth.getFreshToken();
      if (!token) {
        this.errorMessage.set('Authentication token unavailable');
        this.isLoading.set(false);
        return throwError(() => new Error('Authentication token unavailable'));
      }

      const authRequest = request.pipe(
          map((req: any) => {
      ...
      this.errorMessage.set('Failed to authenticate request');
      this.isLoading.set(false);
      return throwError(() => error);
    }
  }

  async submitPrediction(prediction: Prediction): Promise<Observable<Prediction>> {
    console.log(`Submitting prediction: ${JSON.stringify(prediction)}`);
    return (await this.executeRequest(
        this.http.post<Prediction>(`${this.apiUrl}/${this.currentUserId()}/submit`, prediction),
  ...
  async getPredictions(userId: string): Promise<Observable<Prediction[]>> {
    return this.executeRequest(
        this.http.get<Prediction[]>(`${this.apiUrl}/${userId}/predictions`, {
  ...
  async getGamesForPredictions(): Promise<Observable<Game[]>> {
    const userId = this.currentUserId();
    if (!userId) {
      this.errorMessage.set('User not authenticated');
      this.isLoading.set(false);
    ...
    return this.executeRequest(
        this.http.get<Game[]>(`${this.apiUrl}/${userId}/games`, {
          withCredentials: true
        }).pipe(
            map(games => {
