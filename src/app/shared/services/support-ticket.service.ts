...
import {SortColumn, SortDirection} from '../../shared/directives/sor-table.directive';
...
function sort(support: any[], column: SortColumn, direction: string): any[] {
  if (direction === '' || column === '') {
    return support;
  } else {
...
function matches(ticket: any, term: string, pipe: PipeTransform) {
...
export class SupportTicketService {
  ...
  private _support$ = new BehaviorSubject<any[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let support = sort(any, sortColumn, sortDirection);
